...
Static site for [Revenue Durability Group](https://revenuedurability.com). No build step, no JS framework. Deploys to GitHub Pages, Netlify, Vercel, Cloudflare Pages, or any static host in under 5 minutes.

## Vocabulary

Three signature terms — each appears throughout the site and should be used consistently:

- **The Anatomy of Revenue Durability™** — the five-pillar model (was "the Framework")
- **The Revenue Half-Life™** — the 0–100 diagnostic score (was "the Revenue Durability Score")
- **Deep-Roots Customer** — the right-fit customer profile (was "ICP" / "Ideal Customer Profile"). Positive shorthand: "Deep-Roots." Negative shorthand: "wrong-fit."

## Favicon & app icons

The site ships an SVG favicon that matches the nav brand-mark: rounded square with the cyan→emerald brand gradient and a dark up-and-to-the-right arrow.

Files:
- `assets/img/favicon.svg` — primary favicon (32×32 viewBox, scales to any tab size)
- `assets/img/apple-touch-icon.svg` — 180×180 iOS variant, more padding
- Every HTML page's `<head>` wires both, plus `<meta name="theme-color" content="#050B17">` so mobile browser chrome matches the site background.

SVG favicons work in Chrome, Firefox, Safari, and Edge — that's ~99% of your traffic. If you want maximum compatibility (IE11 fallback, older iOS home-screen icons that don't accept SVG), generate a full icon set:

1. Upload `assets/img/favicon.svg` to [realfavicongenerator.net](https://realfavicongenerator.net/)
2. Download the pack
3. Drop the PNG / ICO files into `assets/img/`
4. Add corresponding `<link>` tags to each page's head — the site is small enough that a find-and-replace across the 7 HTML files takes 60 seconds

## Brand & visual system

Dark-mode-first, drawn from the RDG logo:

- **Foundation:** deep navy-black `#050B17` → elevated dark surfaces `#0A1424` → `#0E1A2E`
- **Signature gradient:** blue `#3B82F6` → cyan `#22D3EE` → emerald `#10D680` — mirrors the logo's "Durability" wordmark treatment
- **Primary CTA:** emerald `#10D680` (matches the strongest color in the logo)
- **Body text:** near-white `#F1F5F9` on the dark foundation
- **Score bands** (recolored for dark backgrounds): leaky red, surviving amber, durable blue, compounding emerald

The gradient shows up as: a hairline at the bottom of every hero, the top edge of pillar cards, active nav underline, quiz progress bar, primary score number, the brand-mark badge, and — subtly — as a radial glow behind hero sections.

### Wordmark

The nav shows an **HTML/SVG wordmark**, not the raster logo, because:

1. The transparent-PNG logo has dark navy "Revenue" and "GROUP" text that disappears on the dark background.
2. A native wordmark scales sharp at any size and matches the site's rendering quality.
3. The logo's own emphasis pattern ("Durability" in gradient) is preserved: `Revenue` stays white, `Durability` shows the brand gradient, `Group` sits as small caps.

The raster logo files are still in `assets/img/` — use them for social cards, email signatures, or anywhere you need the actual mark on a light background. If you want the raster logo back in the nav, swap the SVG brand-mark for `<img src="assets/img/logo.png" alt="RDG">` and add a light-tinted container.

## Structure

```
website/
├── index.html            Homepage — hero, narrative, Anatomy teaser, CTAs
├── framework.html        The Anatomy of Revenue Durability™ deep dive (URL kept as framework.html to avoid breaking links)
├── services.html         Services, offer ladder, engagement phases, pricing
├── about.html            About + contact form + booking CTA
├── quiz.html             Lite 10-question Revenue Half-Life™ quiz
...
**To edit The Anatomy text:** `framework.html` (URL slug kept for link stability) mirrors the structure of the framework doc. Section headings map to what's in `RDG_Revenue_Durability_Framework_v0.1.docx`. Note the doc still uses the older "Framework / Score / ICP" vocabulary — refresh it when you have a spare hour, or ask me to sync the doc.
...
The strings "Revenue Durability™", "The Revenue Half-Life™", and "The Anatomy of Revenue Durability™" appear throughout the site. If/when the marks are formally registered (®), update the symbols site-wide with a find-and-replace.

## License

Site content and copy © Revenue Durability Group. The Anatomy, methodology, and diagnostic questions are proprietary — reuse requires permission.
